...
The BWR algorithm is a parameter-free optimization algorithm that leverages the best solution,
worst solution, and a randomly selected solution to guide the search process.
"""
...
class BWROptimizer:
    ...
    BWR uses the formula:
    V' = V + r₁·(Best - T·Random) - r₂·(Worst - Random)
    
    where V is the current solution, Best is the best solution, Worst is the worst solution,
    ...
    def __init__(
        ...
        Args:
            ...
            T: Parameter controlling the influence of the random solution (default: 1.0)
    ...
    def _evaluate_population(self) -> np.ndarray:
        ...
        for i in range(self.population_size):
            ...
            # Update worst individual if worse
            if fitness_values[i] < self.worst_fitness:
                self.worst_fitness = fitness_values[i]
                self.worst_individual = self.population[i].copy()
    ...
    def evolve(self) -> Tuple[np.ndarray, float]:
        ...
        # Get best and worst individuals
        ...
        for i in range(self.elitism, self.population_size):
            ...
            if np.random.random() >= 0.5:  # Exploitation
                ...
                new_population[i] = individual + \
                                   r1 * (self.best_individual - self.T * random_individual) - \
                                   r2 * (self.worst_individual - random_individual)
    ...
    def get_worst_individual(self) -> np.ndarray:
        """Return the worst individual found in the current generation."""
    ...
    def get_population_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "repulsion": np.mean([
                np.linalg.norm(ind - self.worst_individual) 
                for ind in self.population
            ]),
        }
...
class BWRMergedModel(nn.Module):
    """
    A model created by merging multiple models using BWR algorithm weights.
    This is a production-ready implementation that works with real models.
    """
    def __init__(self, base_models, weights):
        """
        Initialize a merged model using BWR weights.
        ...
        self.logger.info(f"Initializing BWRMergedModel with {len(base_models)} base models")
    ...
    def _merge_models(self):
        """Merge the models using the weights."""
        self.logger.info("Merging models with BWR weights")
...
def load_best_model(model_path: str):
    """
    Load a model from a BWR checkpoint file.
    ...
    logger.info(f"BWR: Loading model weights from {model_path}")
    
    try:
        # Load the weights from the numpy file
        weights = np.load(model_path)
        logger.info(f"BWR: Successfully loaded weights with shape {weights.shape}")
        
        # Check if we have a merged model saved
        merged_model_path = model_path.replace("_best.npy", "_merged_model")
        if os.path.exists(merged_model_path):
            logger.info(f"BWR: Loading pre-merged model from {merged_model_path}")
            try:
                model = AutoModel.from_pretrained(merged_model_path)
                logger.info(f"BWR: Successfully loaded pre-merged model")
                return model
            except Exception as e:
                logger.warning(f"BWR: Could not load pre-merged model: {e}")
                logger.warning(f"BWR: Will attempt to recreate merged model")
        ...
        logger.warning("BWR: Cannot recreate merged model without base models")
        logger.warning("BWR: Returning a simple model instead")
        ...
        logger.info(f"BWR: Created simple model as fallback")
        return model
        
    except Exception as e:
        logger.error(f"BWR: Error loading model from {model_path}: {e}")
        import traceback
        logger.error(f"BWR: Traceback: {traceback.format_exc()}")
        raise